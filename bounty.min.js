...
$.ajax({url:"search_state.php",type:"POST",data:{short_name:short_name,player:player||"",found:!!found},dataType:"json",success:function(response){GBT.search_state=response;applyState();if(typeof callback==="function"){callback(true);}},error:function(xhr){if(typeof callback==="function"){callback(false,xhr.responseText);}}});};beginAutoSync=function(){if($body.hasClass("demo")){return;}
...
beginAutoSync();};$("#register").submit(function(){var form=this,$submit_button=$(form).find('input[type="submit"]');$submit_button.addClass("working");$.ajax({url:form.action,type:form.method,data:{name:form.guildname.value,admin_email:form.admin_email.value,admin_pw:form.admin_pw.value,admin_pw_confirm:form.admin_pw_confirm.value,member_pw:form.member_pw.value},dataType:"json",success:function(response){GBT.guild_data=response.guild_data;logIn(true);window.alert("Success!");},error:function(xhr){window.alert(xhr.responseText);},complete:function(){$submit_button.removeClass("working");}});return false;});$("#login").submit(function(){var form=this,$submit_button=$(form).find('input[type="submit"]');$submit_button.addClass("working");$.ajax({url:form.action,type:form.method,data:{guildname:form.guildname.value,password:form.password.value},dataType:"json",success:function(response){GBT.guild_data=response.guild_data;GBT.search_state=response.search_state;logIn(response.is_admin);form.password.value="";},error:function(xhr){if(xhr.status===403){window.alert("Invalid guild name or password.");}else{window.alert(xhr.status+" "+xhr.statusText);}},complete:function(){$submit_button.removeClass("working");}});return false;});$("#log-out").click(function(){var $button=$(this);$button.addClass("working");$.ajax({url:"logout.php",type:"POST",success:function(){$body.removeClass("admin logged-in");window.location.hash="";clearTimeout(sync_timer);document.title=base_doc_title;},error:function(xhr){window.alert(xhr.statusCode());},complete:function(){$button.removeClass("working");}});});$("#reset-password").submit(function(){var form=this,$submit_button=$(form).find('input[type="submit"]');$submit_button.addClass("working");$.ajax({url:form.action,type:form.method,data:{admin_email:form.admin_email.value},dataType:"json",success:function(response){window.location.hash="";window.alert("Your password has been reset. Please check your email.");},error:function(xhr){if(xhr.status===404){window.alert("Couldn't find a guild named\n\n"+
form.guildname.value+"'");}else{window.alert(xhr.status+" "+xhr.statusText);}},complete:function(){$submit_button.removeClass("working");}});return false;});$("#options").submit(function(){var form=this,$submit_button=$(form).find('input[type="submit"]'),data={};$submit_button.addClass("working");if(form.member_pw.value){data.member_pw=form.member_pw.value;}
...
$.ajax({url:form.action,type:form.method,data:data,dataType:"json",success:function(response){GBT.guild_data.member_pw=response.member_pw;GBT.guild_data.admin_email=response.admin_email;form.admin_pw.value="";form.admin_pw_confirm.value="";window.location.hash="";},error:function(xhr){window.alert("Unable to save options.\n\n"+"Error "+xhr.status+": "+xhr.statusText);},complete:function(){$submit_button.removeClass("working");}});return false;});$(window).on("hashchange",function(){var hash=window.location.hash,$input;if(!hash||hash==="#"){return;}
